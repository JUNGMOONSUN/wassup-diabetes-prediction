...
class HealthDataOrigin:
    ...
    def preprocess(self):
        ...
        enc = OneHotEncoder(
            dtype=np.float32, sparse=False, drop="if_binary", handle_unknown="ignore"
        )
...
def get_args_parser(add_help=True):
    ...
    parser = argparse.ArgumentParser(
        description="Data preprocessing", add_help=add_help
    )
    # inputs
    parser.add_argument(
        "--train-csv", default="./data/train.csv", type=str, help="train data csv file"
    )
    parser.add_argument(
        "--test-csv", default="./data/test.csv", type=str, help="test data csv file"
    )
    # outputs
    parser.add_argument(
        "--output-train-feas-csv",
        default="./trn_X.csv",
        type=str,
        help="output train features",
    )
    parser.add_argument(
        "--output-test-feas-csv",
        default="./tst_X.csv",
        type=str,
        help="output test features",
    )
    parser.add_argument(
        "--output-train-target-csv",
        default="./trn_y.csv",
        type=str,
        help="output train targets",
    )
    parser.add_argument(
        "--output-test-target-csv",
        default="./tst_y.csv",
        type=str,
        help="output test targets answer",
    )  # tst_y.csv(정답지)
    # options
